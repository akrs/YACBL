    ...
    generator: {
        java: -> # TODO: check if type of generator is binary expression or iterable object
            if (@generator.op is "..<" or @generator.op is "...")
                return "for (int _#{@innerId.generator.java()} = #{@generator.leftSide.generator.java()}; i #{if @iterator.op is '..<' then '<' else '<='} #{@iterator.op.rightSide.generator.java()}; i++) #{@block.generator.java()}"
    }
